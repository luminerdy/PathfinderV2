...
```bash
ls skills/
# Should show: arm_control.py  auto_pickup.py  block_approach.py  
#              block_detect.py  block_pursue.py  centering.py  strafe_nav.py
```
...
| Flask | 3.1+ | Web control interface |
| PySerial | 3.5+ | Serial communication (Pi 5) |
| SMBus2 | 0.4+ | I2C communication (Pi 4) |
| PyYAML | 6+ | Configuration files |
| NumPy | 2+ | Math operations |
...
- Check `dtoverlay=disable-bt` in boot config
...
*Tested on: Raspberry Pi 4 Model B Rev 1.5, Raspberry Pi OS Bookworm 64-bit*
