...
class Function(IntEnum):
    ...
    MOTOR = 0x01        # Motor speed/duty control
    SERVO = 0x02        # PWM servo position
    RGB = 0x03          # RGB LED control
    BUZZER = 0x04       # Buzzer control
    BATTERY = 0x05      # Battery voltage query
    SONAR = 0x06        # Sonar distance (not used - I2C)
...
class BoardController:
    ...
    def set_buzzer(self, freq_hz: int, on_ms: int, off_ms: int, repeat: int = 1):
        ...
        """
        data = struct.pack("<HHHH", freq_hz, on_ms, off_ms, repeat)
    ...
    def get_battery(self) -> Optional[int]:
        ...
        """
        # Send query
        self.protocol.send_command(Function.BATTERY, bytes())
        
        # Wait for response
        response = self.protocol.read_response(timeout=0.5)
        if response and response[0] == Function.BATTERY:
            data = response[1]
            if len(data) >= 2:
